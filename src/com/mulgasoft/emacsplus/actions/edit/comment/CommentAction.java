...
public abstract class CommentAction extends ReversibleMultiCaretInsightAction
{
    ...
    protected boolean isValidFor(@NotNull final Project project, @NotNull final Editor editor, @NotNull final Caret caret, @NotNull final PsiFile file) {
        ...
        if (editor == null) {
            throw new IllegalArgumentException(String.format("Argument for @NotNull parameter '%s' of %s.%s must not be null", "editor", "com/mulgasoft/emacsplus/actions/edit/comment/CommentAction", "isValidFor"));
        }
        if (caret == null) {
            throw new IllegalArgumentException(String.format("Argument for @NotNull parameter '%s' of %s.%s must not be null", "caret", "com/mulgasoft/emacsplus/actions/edit/comment/CommentAction", "isValidFor"));
        }
        if (file == null) {
            throw new IllegalArgumentException(String.format("Argument for @NotNull parameter '%s' of %s.%s must not be null", "file", "com/mulgasoft/emacsplus/actions/edit/comment/CommentAction", "isValidFor"));
        }
        ...
        if (!ISHandler.isInISearch(editor)) {
            ...
            else if (LanguageCommenters.INSTANCE.forLanguage(file.getLanguage()) != null || LanguageCommenters.INSTANCE.forLanguage(file.getViewProvider().getBaseLanguage()) != null) {
                result = true;
            }
            else {
